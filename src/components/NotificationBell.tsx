...
export default function NotificationBell() {
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      ...
      // silent fail — bell shouldn't break dashboard
    }
  }, []);
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-80 rounded-xl border border-[var(--border)] bg-[var(--card)] shadow-xl z-50">
          <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border)]">
            <h3 className="text-sm font-semibold text-[var(--card-foreground)]">Notifications</h3>
            {unreadCount === 0 && (
              <span className="text-xs text-[var(--muted-foreground)]">All caught up</span>
            )}
          </div>
          ...
          <ul className="max-h-72 overflow-y-auto divide-y divide-[var(--border)]">
            {notifications.length === 0 ? (
              ...
              notifications.map((n) => (
                ...
                >
                  <p className="text-sm text-[var(--card-foreground)]">{n.message}</p>
